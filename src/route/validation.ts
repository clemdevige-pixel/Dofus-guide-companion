...
const supportedTypes = new Set<StepType>([
  'quest', 'resume', 'dungeon', 'preparation', 'rule', 'milestone', 'long_running',
  'hard_lock', 'alignment', 'order', 'major_step', 'finish',
]);
...
export function validateRoute(route: RouteDocument): RouteDocument {
  if (route.schemaVersion !== 1) throw new Error(`Version de schéma non supportée : ${route.schemaVersion}`);
  ...
  if (!Array.isArray(route.blocks) || route.blocks.length === 0) throw new Error('RouteDocument sans bloc.');
  if (!Array.isArray(route.steps) || route.steps.length === 0) throw new Error('RouteDocument sans étape.');
  ...
  for (let index = 0; index < route.blocks.length; index += 1) {
    ...
    if (blockIds.has(block.id)) throw new Error(`blockId dupliqué : ${block.id}`);
    blockIds.add(block.id);
  }

  const stepIds = new Set<string>();
  const goalStates = new Map<string, GoalState>();
  const parallelStates = new Map<string, GoalState>();
  ...
  for (let index = 0; index < route.steps.length; index += 1) {
    ...
    if (stepIds.has(step.id)) throw new Error(`stepId dupliqué : ${step.id}`);
    stepIds.add(step.id);

    if (!blockIds.has(step.blockId)) throw new Error(`${step.id}: blockId inconnu (${step.blockId})`);
    if (!supportedTypes.has(step.type)) throw new Error(`${step.id}: type inconnu (${String(step.type)})`);
    ...
    if (step.parallelGroup) {
      if (!isNonEmptyString(step.parallelGroup.parallelId)) {
        throw new Error(`${step.id}: parallelId vide.`);
      }
      if (!supportedParallelPhases.has(step.parallelGroup.phase)) {
        throw new Error(`${step.id}: parallel phase inconnue (${String(step.parallelGroup.phase)}).`);
      }

      const parallelId = step.parallelGroup.parallelId;
      const state = parallelStates.get(parallelId);
      if (step.parallelGroup.phase === 'start') {
        if (state) throw new Error(`${step.id}: parallelId démarré plusieurs fois (${parallelId}).`);
        parallelStates.set(parallelId, 'active');
        parallelStarts.set(parallelId, index);
      } else if (step.parallelGroup.phase === 'progress') {
        if (state !== 'active') {
          throw new Error(`${step.id}: progression parallèle avant start ou après finish (${parallelId}).`);
        }
      } else if (step.parallelGroup.phase === 'finish') {
        if (state !== 'active') {
          throw new Error(`${step.id}: fin parallèle avant start ou après finish (${parallelId}).`);
        }
        parallelStates.set(parallelId, 'finished');
        parallelFinishes.set(parallelId, index);
      }
    }
    ...
    if (step.guideItems !== undefined) {
      ...
      for (let guideIndex = 0; guideIndex < step.guideItems.length; guideIndex += 1) {
        ...
        if (!isNonEmptyString(item.label)) throw new Error(`${context}: libellé vide.`);
        assertValidCoordinate(item.location, context, 'position');
        if (item.note !== undefined && !isNonEmptyString(item.note)) throw new Error(`${context}: note vide.`);
      }
    }
    ...
    if (goal) {
      if (!isNonEmptyString(goal.goalId)) throw new Error(`${step.id}: goalId vide.`);
      const state = goalStates.get(goal.goalId);
      if (goal.phase === 'start') {
        if (state) throw new Error(`${step.id}: goalId démarré plusieurs fois (${goal.goalId}).`);
        goalStates.set(goal.goalId, 'active');
      } else if (goal.phase === 'progress') {
        if (state !== 'active') throw new Error(`${step.id}: progress avant start ou après finish (${goal.goalId}).`);
      } else if (goal.phase === 'finish') {
        if (state !== 'active') throw new Error(`${step.id}: finish avant start ou après finish (${goal.goalId}).`);
    ...
    if (step.hardLock?.goalId && !goalStates.has(step.hardLock.goalId)) {
      throw new Error(`${step.id}: verrou lié à un goalId pas encore démarré (${step.hardLock.goalId})`);
    }

    if (step.type === 'finish') {
      finishCount += 1;
      if (index !== route.steps.length - 1) throw new Error(`${step.id}: FIN doit être la dernière étape de la route.`);
    }
  }

  for (const [parallelId, state] of parallelStates) {
    if (state !== 'finished') throw new Error(`Groupe parallèle non terminé : ${parallelId}.`);
    if (!parallelStarts.has(parallelId) || !parallelFinishes.has(parallelId)) {
      throw new Error(`Lifecycle parallèle incomplet : ${parallelId}.`);
    }
  }
